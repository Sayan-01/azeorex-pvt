  ...
  return (
    <aside className="w-64 border-r border-zinc-800 flex flex-col">
      <div className="p-5 flex flex-col gap-y-1">
        ...
        {items.map((item) => (
          <div
            className={`flex items-center justify-between py-2 px-3 hover:bg-zinc-800 cursor-pointer rounded-xl ${selectedNode === item.id ? "bg-zinc-800" : ""}`}
            onClick={() => router.push(`/saas/projects/${projectId}/cms?node=${item.id}`, { scroll: false })}
            key={item.id}
          >
